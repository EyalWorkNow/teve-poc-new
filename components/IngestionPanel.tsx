...
interface IngestionPanelProps {
  onAnalyze: (text: string, title: string, media: RawMedia[]) => void;
  isAnalyzing: boolean;
  onCancel: () => void;
}
...
const IngestionPanel: React.FC<IngestionPanelProps> = ({ onAnalyze, isAnalyzing, onCancel }) => {
  ...
  const handleFileSelect = (file: File) => {
      ...
      const newMedia: RawMedia = {
          ...
          content: type === 'text' || type === 'log' ? "Preview of uploaded file content..." : undefined,
          metadata: {
      ...
      setRawContent(prev => {
          ...
          return prefix + `[SYSTEM ATTACHMENT]\nFILENAME: ${file.name}\nTYPE: ${type.toUpperCase()}\nMETADATA: ${file.size} bytes`;
      });
  };
  ...
  const loadScenario = (type: 'RED_SEA' | 'CYBER' | 'HUMINT') => {
      ...
      if (type === 'RED_SEA') {
          ...
          setRawContent(`INTERCEPTED SIGNAL: 4922-Alpha
...
Drone Swarm (Samad-3) launched from inland launch site towards Eilat as diversion.`);
          ...
          setRawContent(`INCIDENT REPORT: Ziv Medical Center
...
Traffic analysis shows C2 (Command & Control) server located in Tehran.`);
          ...
          setRawContent(`SUBJECT: Abu Ali (Courier)
...
${rawContent}
      ...
      onAnalyze(enrichedContent, title || `Ingestion ${timestamp}`, uploadedMedia);
  };
  ...
  useEffect(() => {
    if (isAnalyzing) {
        setLogLines([]);
        // Added Biometric Scan Steps
        const messages = [
            "INITIALIZING CONTEXT ENGINE...",
            "TOKENIZING INPUT STREAM...",
            "PROCESSING MEDIA ATTACHMENTS...",
            ...(runFaceScan ? ["INITIATING BIOMETRIC SCAN [FACIAL_REC]...", "COMPARING AGAINST KELASTRON DATABASE..."] : []),
            ...(runVoiceScan ? ["ANALYZING AUDIO WAVEFORMS [VOICEPRINT_V2]...", "EXTRACTING SPEAKER EMBEDDINGS..."] : []),
            "IDENTIFYING ENTITIES [NER_V2]...",
            "DETECTING RELATIONS...",
            "BUILDING KNOWLEDGE GRAPH...",
            "CROSS-REFERENCING INTELLIGENCE DATABASE...",
            "GENERATING TACTICAL INSIGHTS...",
            "FINALIZING INTELLIGENCE PACKAGE..."
        ];
  ...
  }, [isAnalyzing, runFaceScan, runVoiceScan]);

  return (
    <div 
        className="flex flex-col h-full bg-[#181818] relative animate-fadeIn overflow-hidden selection:bg-[#05DF9C]/30 selection:text-white"
    ...
    >
      ...
      {/* ANALYZING OVERLAY */}
      {isAnalyzing && (
          <div className="absolute inset-0 z-[100] bg-[#181818] flex flex-col items-center justify-center font-mono">
              <div className="w-[500px] bg-[#121212] border border-slate-800 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
      ...
      {/* HEADER */}
      <div className="h-20 border-b border-slate-800/50 bg-[#121212]/80 backdrop-blur flex items-center justify-between px-8 shrink-0 z-20">
        <div>
           <h1 className="text-xl font-bold text-white flex items-center gap-3 tracking-tight">
              <div className="p-2 bg-[#05DF9C]/10 rounded-lg border border-[#05DF9C]/20">
      ...
      {/* STEP INDICATOR */}
      <div className="pt-6 pb-2 bg-[#181818] sticky top-0 z-10 shrink-0">
      ...
      <div className="flex-1 overflow-hidden px-4 pb-4 pt-2">
         <div className="max-w-4xl mx-auto h-full flex flex-col">
            ...
            {/* CARD CONTAINER */}
            <div className="flex-1 bg-[#121212] border border-slate-800 rounded-2xl shadow-2xl relative overflow-hidden flex flex-col transition-all duration-500">
                {/* Top Gradient Line */}
